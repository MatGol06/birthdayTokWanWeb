...
export default function TvDisplayMode({ wishes, onOpenForm, onOpenAdmin }) {
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col justify-between pt-16 pb-6 px-3 sm:px-6 md:p-8 lg:p-10 bg-[#140E0A] text-[#FAF0D7] selection:bg-[#D4AF37] selection:text-black">
      {/* Ambient Glow */}
      ...
      {/* TOP MARQUEE HEADER */}
      <header className="relative z-20 max-w-6xl mx-auto w-full mb-4 border-b-2 border-[#D4AF37]/30 pb-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-3 text-center md:text-left">
          ...
          <div className="text-center px-2">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-[#241A13] border border-[#D4AF37]/40 text-[#D4AF37] text-[10px] font-typewriter uppercase tracking-widest mb-1.5">
            ...
            <motion.p 
              ...
              className="text-xs sm:text-base md:text-lg font-heading text-[#FAF0D7] italic mt-1"
            >
              Sambutan Ulang Tahun Ke-64 • <span className="text-[#D4AF37] font-normal font-sans">1 Ogos 2026</span>
            </motion.p>
          </div>

          <div className="text-right flex items-center justify-center gap-2.5 bg-[#241A13] px-3 py-1.5 rounded-xl border border-[#D4AF37]/40 shadow-inner">
            <Clock className="w-3.5 h-3.5 text-[#D4AF37] animate-pulse" />
            <div className="font-heading font-bold text-xs sm:text-sm text-[#FAF0D7]">
        ...
        <div className="mt-4 flex items-center justify-center gap-2 flex-wrap">
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-[11px] sm:text-xs font-bold font-typewriter transition-all cursor-pointer ${
          ...
          {displayCategory === 'wishes' ? (
            currentWish ? (
              <motion.div 
                ...
                className="w-full relative bg-[#261D16] border-2 border-[#D4AF37] rounded-2xl p-4 sm:p-6 md:p-10 shadow-[0_20px_50px_rgba(0,0,0,0.9)] overflow-hidden"
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                  {currentWish.photo ? (
                    <div className="md:col-span-5 flex justify-center">
                      <div className="relative p-1.5 rounded-xl bg-[#1A1008] border-2 border-[#D4AF37] shadow-xl">
                        <img 
                          src={currentWish.photo} 
                          alt={currentWish.sender}
                          className="w-44 h-52 sm:w-56 sm:h-64 object-cover rounded-lg sepia-[0.2]"
                        />
                        <div className="mt-1.5 text-center text-[10px] font-typewriter text-[#D4AF37] flex items-center justify-center gap-1">
                  ...
                  <div className={currentWish.photo ? "md:col-span-7 text-left" : "md:col-span-12 text-center max-w-2xl mx-auto"}>
                    <blockquote className="text-base sm:text-xl md:text-2xl font-heading text-[#FAF0D7] leading-relaxed italic mb-4">
                      "{currentWish.message}"
                    </blockquote>

                    <div className="pt-3 border-t border-[#D4AF37]/20 flex items-center justify-between flex-wrap gap-2">
                      <div>
                        <h3 className="text-lg sm:text-2xl font-bold font-cinema text-gold-gradient">
                          {currentWish.sender}
                        </h3>
                        <p className="text-[11px] sm:text-xs text-[#A89578] font-typewriter mt-0.5">
                          {currentWish.relationship}
                        </p>
                      </div>

                      <div className="text-[10px] sm:text-xs text-[#D4AF37] font-typewriter px-2.5 py-1 rounded border border-[#D4AF37]/40 bg-[#1A1008] flex items-center gap-1">
                        <Heart className="w-3 h-3 text-[#8C1C1C] fill-current" /> SEKELUARGA
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="absolute left-1.5 top-1/2 -translate-y-1/2 p-1.5 rounded-full bg-black/60 border border-[#D4AF37]/40 text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black transition-colors"
                >
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <button
                  onClick={() => setCurrentIndex((prev) => (prev + 1) % wishes.length)}
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 p-1.5 rounded-full bg-black/60 border border-[#D4AF37]/40 text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black transition-colors"
                >
            ...
            currentPhoto ? (
              <motion.div 
                ...
                transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }}
                className="w-full relative bg-[#261D16] border-2 border-[#D4AF37] rounded-2xl p-4 sm:p-6 shadow-2xl text-center"
              >
                <div className="relative max-h-[45vh] rounded-xl overflow-hidden bg-black mb-3 inline-block border-2 border-[#D4AF37]/40">
                  <img 
                ...
                <h3 className="text-base sm:text-xl font-bold font-heading text-[#FAF0D7]">
                  "{currentPhoto.caption}"
                </h3>
                <p className="text-[11px] sm:text-xs text-[#D4AF37] font-typewriter mt-1">
      ...
      {/* FOOTER CONTROLS */}
      <footer className="relative z-30 max-w-6xl mx-auto w-full flex flex-col md:flex-row items-center justify-between gap-4 pt-3 border-t border-[#D4AF37]/30">
        <GramophonePlayer />

        <div className="flex items-center gap-2.5 w-full md:w-auto justify-center">
          <button
            onClick={() => setShowQrModal(!showQrModal)}
            className="flex-1 md:flex-initial flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl bg-[#241A13] border border-[#D4AF37] text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black font-bold text-xs uppercase tracking-wider transition-all shadow-lg cursor-pointer"
          >
            <QrCode className="w-4 h-4" /> Impas QR
          </button>

          <button
            onClick={onOpenForm}
            className="flex-1 md:flex-initial flex items-center justify-center gap-1.5 px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#B8860B] via-[#FFD700] to-[#996515] text-black font-bold text-xs uppercase tracking-wider shadow-xl hover:brightness-110 active:scale-95 transition-all cursor-pointer font-heading"
          >
            <MessageSquare className="w-4 h-4" /> Hantar Ucapan
          </button>
        </div>
      </footer>
      ...
      {showQrModal && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#241A13] border-2 border-[#D4AF37] rounded-2xl p-6 sm:p-8 max-w-md w-full text-center relative shadow-[0_0_50px_rgba(212,175,55,0.5)]">
            <button
            ...
            <div className="bg-white p-3 rounded-xl inline-block shadow-2xl mb-3 border-2 border-[#D4AF37]">
              <QRCodeSVG value={currentUrl} size={180} />
            </div>
            <p className="text-[11px] text-[#D4AF37] font-semibold break-all bg-black/40 p-2.5 rounded-lg border border-[#D4AF37]/30">
              {currentUrl}
            </p>
          </div>
        </div>
